#This file is identical to Tax_data3.R, however uses old tax brackets from 2017
...
single_tax_totals <- sapply(1:length(indv2), function(n) 
  c(sapply(indv1, single_tax2017) + sapply(indv2[n], single_tax2017))
)
...
png(file="Total_Tax_Owed_2017.png", width = 900, height = 600)
draw(htmp_single + htmp_joint, column_title = "2017 Total Tax Owed")
dev.off()
...
htmp_bonus <- Heatmap(bonus_penalty,
                      cluster_columns=FALSE,
                      column_title = "2017 Marriage Tax Bonus: Joint Filing vs Single Filing",
                      cluster_rows=FALSE,
                      row_names_gp = gpar(col = 
                                            c(
                                              rep(
...
png(file="htmp_bonus_2017.png", width = 800, height = 800)
draw(htmp_bonus)
dev.off()
...
write.csv(income_data, "tax_data_2017.csv")
...
png(file="ContributionPlot_2017.png", width = 800, height = 800)
draw(htmp_bonus_percent, row_title ="Total Joint Income ($)", column_title ="2017 Marriage Tax Bonus: Joint Filing vs Single Filing")
dev.off()
